...
module.exports = new function() {
    ...
    function rebuildIndex(options) {
        ...
        if(options.rebuildMatch) {
            ...
            for(var i in matchedFiles) {
                ...
                // Clear informations about this file.
                importedFiles[matchedFile] = [];
                delete compiledFiles[matchedFile];
                delete sourceMapFiles[matchedFile];
                delete statFiles[matchedFile];
            }
            ...
            for(var i in newRootFiles) {
                ...
                // Store file fs.Stat.
                statFiles[indexFile] = fs.statSync(indexFile);
    ...
    function watchLess() {
        ...
        chokidar.watch(process.cwd(), { "alwaysStat": true })
            .on("change", function(filename, stats) {
                // Ignore files not changed.
                var originalStatFile = statFiles[getRelativePath(filename)];
                if(!originalStatFile
                || +originalStatFile.mtime === +stats.mtime) {
                    return;
                }
