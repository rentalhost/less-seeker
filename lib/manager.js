...
module.exports = new function() {
    ...
    function rebuildIndex(options) {
        ...
        if(options.fileAdded) {
            options.rebuildMatch = options.fileAdded;
        }
                    ...
                    us.each(newRootFiles, function(filename) {
                        ...
                        switch(true) {
                            ...
                            case options.firstScan:
                                ...
                                break;
                            // Case is fileAdded:
                            case !!options.fileAdded && isRootFile(filename):
                        ...
                        // If file was added.
                        if(options.fileAdded === filename
                        && !isRootFile(filename)) {
                            rootFileMessage+= cli.yellowBright(" (new)");
                        }
                        else
                        ...
                        if(!rootWasRemoved) {
                            ...
                            if(args.autoClean) {
                                unlinkFile(getSourceMapFilename(filename));
                            }
                        }
                        else
                        // If root was removed and auto-clean is enabled.
                        if(args.autoClean) {
                            unlinkFile(getCSSFilename(filename));
                            unlinkFile(getSourceMapFilename(filename));
                        }
    ...
    function fileAdded(filename) {
        if(path.extname(filename) === ".less") {
            rebuildIndex({ "fileAdded": getRelativePath(filename) });
        }
    }
